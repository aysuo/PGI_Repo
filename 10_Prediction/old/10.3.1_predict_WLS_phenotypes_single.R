...
# scores
score_wd <- "/disk/genetics/PGS/Aysu/PGS_Repo_pipeline/derived_data/9_Scores/single/scores/"
score_files <- list.files(score_wd)
WLS_files_all <- score_files[grep("WLS", score_files)]
score_names <- gsub("PGS_WLS_", "", gsub("-single_LDpred_p1.txt", "", WLS_files_all))
...
# PCs
PCs_path <- "/disk/genetics/PGS/Aysu/PGS_Repo_pipeline/derived_data/8_PCs/WLS/WLS_PCs.eigenvec"
PCs_oldnames <- paste0("V", 3:22)
PCs_newnames <- paste0("pc", 1:20)
PCs_data <- fread(PCs_path, fill=TRUE) %>%
  rename(IID = V2) %>%
...
for (i in 1:length(pheno_names)){
  ...
  pheno_path <- paste0(getwd(), "/input/WLS/", pheno, ".pheno")
  score_path <- paste0(score_wd, WLS_files_all[grep(paste0(pheno, "-"), WLS_files_all, fixed=TRUE)])
  ...
  score_data <- score_data %>%
    ...
    filter(FID != "NA" & IID != "NA") %>%
    mutate(id = as.numeric(IID))

  # join data with each other and PCs
  data <- pheno_data %>%
    inner_join(score_data, by="id") %>%
    inner_join(PCs_data, by="IID") %>%
    select(id, phenotype, score, paste0("pc", 1:10)) %>%
    drop_na()
  ...
  local_bootstrap_df$inc_r2_single <- r2_inc_list
...
fwrite(
  ...
  paste0(
    getwd(),
    "/output/WLS_bootstraps_single.txt"
  )
)
...
fwrite(df,
  paste0(getwd(),
    "/output/WLS_phenotypes_r2_single.txt")
  )
