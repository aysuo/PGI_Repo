#----------------------------------------------------------------------------------#
# Predicts WLS phenotypes from scores created using public data
# Date: 04/16/2020
# Author: Joel Becker
...
score_names <- gsub("PGS_WLS_", "", gsub("_LDpred_p1.txt", "", WLS_files_all))
...
for (i in 1:length(pheno_names)){
  ...
  local_score_path <- WLS_files_all[grep(paste0(pheno, "-"), WLS_files_all, fixed=TRUE)]
  score_path <- paste0(score_wd, local_score_path)

  pheno_data <- fread(pheno_path)
  score_data <- fread(score_path)

  colnames(score_data) <- toupper(colnames(score_data))

  score_data <- score_data %>%
      rename(score = contains(substr(pheno, 1, 4))) %>%
...
fwrite(
  ...
  paste0(
    getwd(),
    "/output/WLS_bootstraps_public.txt"
  )
)
...
fwrite(
  ...
  paste0(
    getwd(),
    "/output/WLS_phenotypes_r2_public.txt"
  )
)
