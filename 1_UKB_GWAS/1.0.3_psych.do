...
gen ASD_ICD10 = .
forval i = 0/242 {
    foreach j in "F840" "F841" "F845" "F848" "F849" {    
        replace ASD_ICD10 = 2 if s_41270_0_`i' == "`j'"
    }
}
...
forval i = 0/46 {
    foreach j in "2990" {
        replace ASD_ICD9 = 2 if s_41271_0_`i' == "`j'"
    }
}
...
forval i = 0/1 {
    foreach j in "F840" "F841" "F845" "F848" "F849" {      
        replace ASD_ICD10Dm = 2 if s_40001_`i'_0 == "`j'"
    }
}
...
forval i = 1/14 {
    foreach j in "F840" "F841" "F845" "F848" "F849" {    
        replace ASD_ICD10Ds = 2 if s_40002_0_`i' == "`j'"
    }
}
forval i = 1/9 {
    foreach j in "F840" "F841" "F845" "F848" "F849" {    
        replace ASD_ICD10Ds = 2 if s_40002_1_`i' == "`j'"
    }
}
...
gen ASD_D = .
forval i=1/16{
    replace ASD_D = 2 if n_20544_0_`i' == 14
}


replace ASD_GP=. if ASD_GP==0
egen ASD=rmax(ASD_ICD10 ASD_ICD10Dm ASD_ICD10Ds ASD_ICD9 ASD_D ASD_GP PsychCon)
replace ASD = . if ASD==1
replace ASD = 1 if ASD==2
...
*** SAVE FULL DATASET ***
keep n_eid partition FID IID Sex Batch BYEAR ADHD ANOREX ASD BIPOLAR DEP INSOMNIA SCZ PC*

save "tmp/pgi_repo_psych.dta", replace

sum ADHD ANOREX ASD BIPOLAR DEP INSOMNIA SCZ

**********************************************************************************
**********************************************************************************
local psych "ADHD ANOREX ASD BIPOLAR SCZ DEP INSOMNIA"
by partition, sort: summarize `psych'

/* foreach partition in 1 2 3 {
    foreach var of varlist `psych' {
...
/*foreach partition in 1 2 3{
    foreach var of varlist SCZ ADHD ANOREX ASD {
...
qui xi:reg ANOREX BYEAR* Sex* i.Batch PC1-PC40
predict resid, rstandard
export delimited FID IID resid using "input/UKB_ANOREX.pheno", noq delim(" ") replace
