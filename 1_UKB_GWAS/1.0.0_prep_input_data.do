...
local partition2="`3'"
local partition3="`4'"
local pheno_data_1="`5'"
local pheno_data_2="`6'"
local pheno_data_3="`7'"
local covar_data="`8'"
local withdrawn="`9'"
local logfile="`10'"
...
****************************************
* THIS IS HOW THE PARTITIONS WERE MADE INITIALLY, BUT COMMENTING IT OUT SINCE STATA RANDOM VARIABLE GENERATOR HAS CHANGED AND GIVES DIFFERENT OUTPUT.
* USING RELEASE V1.0 PARTITION LISTS INSTEAD
...
clear
import delimited `partition1', delim("\t")
merge 1:1 n_eid using "tmp/pgi_repo.dta", keep(using match)
replace partition=1 if _merge==3
drop _merge
save "tmp/pgi_repo.dta", replace

clear
import delimited `partition2', delim("\t")
merge 1:1 n_eid using "tmp/pgi_repo.dta", keep(using match)
replace partition=2 if _merge==3
drop _merge
save "tmp/pgi_repo.dta", replace

clear
import delimited `partition3', delim("\t")
merge 1:1 n_eid using "tmp/pgi_repo.dta", keep(using match)
replace partition=3 if _merge==3
drop _merge
save "tmp/pgi_repo.dta", replace
