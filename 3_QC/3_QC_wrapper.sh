...
checkStatus(){
	...
	while read file; do
		...
		if [[ $Nplots < $targetN ]]; then
			grep ${pheno}\.txt $fileList >> $PGI_Repo/code/3_QC/${cohort}_${analysis}_rerun
...
QC(){
	...
	for SEfilter in 0 1; do
		...
		ls -d $outDir/QC_* | sed '/log/d' | sed '/ecf/d'> $PGI_Repo/code/3_QC/${cohort}_SE${SEfilter}_QCdirs
		while read dir; do
			if [[ -d $dir ]]; then
...
get_dupN(){
	...
	rm $cohort/SEfilter/duplicates.txt
	for i in $cohort/SEfilter/*; do 
		if [[ -d $i ]] 
		then
			pheno=$(echo $i | cut -d"/" -f3 | cut -d"-" -f1 | sed 's/QC_//g')
			study=$(echo $i | cut -d"/" -f3 | cut -d"-" -f2 | cut -d"_" -f1)
			Ndup=$(wc -l $i/*duplicates* | cut -d" " -f1)
			let dup=dup-2
			echo -e ${pheno}-${study}"\t"${Ndup} >> $cohort/SEfilter/duplicates.txt
		fi
	done
}
...
for cohort in UKB; do #23andMe UKB public; do
	get_dupN $cohort
done
