...
format_UKB(){
	...
	N=$(grep -a "Number of indivs with no missing phenotype(s) to use:" $mainDir/derived_data/1_UKB_GWAS/logs/$pheno/UKB_${pheno}_part${part}_BOLTLMM.log | cut -d" " -f10)
...
main() {
	phenos=$(ls -d $mainDir/derived_data/1_UKB_GWAS/output/* | rev | cut -d"/" -f1 | rev | sed '/^old/d')
