...
mergePGI(){
    ...
    tmp=$(ls -1 $PGI_Repo/derived_data/9_Scores/single/scores/PGS_${cohort}_* | head -1)
    cut -f1,2 $tmp > $PGI_Repo/pgs/tmp_$cohort 
    for PGStype in $PGStypes; do
        for PGS in $(ls -1 $PGI_Repo/derived_data/9_Scores/$PGStype/scores/PGS_${cohort}_*); do 
            awk -F"\t" 'NR==FNR{pgs[$2]=$5;next}{print $0,pgs[$2]}' OFS="\t" $PGS $PGI_Repo/pgs/tmp_$cohort > $PGI_Repo/pgs/tmp2_$cohort
    ...
    sed -i "s/PGS/PGI/g" $PGI_Repo/pgs/${cohort}_*.txt
}
...
fixIDs(){
    ...
    awk -F"\t" 'NR==1{print;next} \
        {split($1,a,/_/) ; $1=a[1]; $2=a[2] ; print}' OFS="\t" $PGI_Repo/pgs/${cohort}_PGI_20PCs.txt > $PGI_Repo/pgs/tmp_$cohort
    mv $PGI_Repo/pgs/tmp_$cohort $PGI_Repo/pgs/${cohort}_PGI_20PCs.txt
}
