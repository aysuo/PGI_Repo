...
# Openness
awk -F"\t" 'BEGIN{OFS="\t"; print "SNPID","CHR","BP","EFFECT_ALLELE","OTHER_ALLELE","EAF","BETA","SE","P","INFO","N","IMPUTED","CALLRATE","HWE_PVAL","PLOIDY"} \
	{N=17375;print $1,"NA","NA",toupper($4),toupper($5),"NA",$6,$7,$8,$9,N,1,1,1,"A"}' OFS="\t" $dirIn/GPC-1.NEO-OPENNESS.full.txt > tmp/OPEN-GPC1.txt &
...
for file in OPEN-GPC1.txt SELFHEALTH-Harris.txt; do
...
## Files with no SE 
for file in SELFHEALTH-Harris.txt; do
	Rscript $dirCode/1.1_Add_SE.R $dirOut/tmp/$file $dirOut/$file &
done
wait

## Files with no Chr BP
for file in OPEN-GPC1.txt DEP-Howard.txt; do
