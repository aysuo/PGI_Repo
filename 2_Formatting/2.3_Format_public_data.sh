...
for file in BMI-Locke.txt AgeFirstMenses-Perry.txt Height-Wood.txt Eversmoke-TAG.txt CPD-TAG.txt; do
...
mv AgeFirstMenses-Perry.txt tmp/tmp_AgeFirstMenses-Perry.txt

## Files with no SE
for file in AgeFirstMenses-Day.txt AgeFirstMenses-Perry.txt; do
	Rscript $dirCode/2.3.1_Add_SE.R $dirOut/tmp/tmp_$file $dirOut/$file &
done
wait
