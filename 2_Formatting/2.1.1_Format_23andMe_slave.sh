...
## Takes 2 args: input file path
##               columns to keep, separated by comma
...
main(){
  ...
  then
    ...
    sh /disk/genetics4/PGS/Aysu/PGS_Repo_pipeline/code/2_Formatting/2.1.1.1_ExtractCols.sh \
    --file ${annot_all} \
    ...
    sh /disk/genetics4/PGS/Aysu/PGS_Repo_pipeline/code/2_Formatting/2.1.1.2_Merge.sh \
    --mergeType L \
    --mergeCol1 3 \
    --mergeCol2 1 \
  ...
  do
    ...
    then
      ...
      awk 'NR==FNR{a[$1]=$1;next}{ChrPosID=$2":"$3}!(ChrPosID in a){print $0}' OFS="\t" /var/genetics/ukb/aokbay/reffiles/1kG_phase1/1000G_flipped_all_SNPID.txt ${OUT[$i]} > TMP/tmp_${gwas_file}
