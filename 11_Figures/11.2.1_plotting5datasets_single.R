#----------------------------------------------------------------------------------#
# Charts differences in single trait and public prediction results
# Date: 04/16/2019
...
# drop dunedin cigerettes per day for better graph legibility
data <- data %>%
    filter(!(phenotype == "CPD" & Dataset == "Dunedin"))

### Panel 1 -------

plot11 <- data %>%
    filter(complete.cases(!!sym(yvar_bar))) %>%
    filter(pheno_group %in% groups[1:3]) %>%
    filter(phenotype == "BMI" | phenotype == "HEIGHT") %>%
    ggplot(aes(fill=Dataset, x = phenotype_n, 
               y = !!sym(yvar_bar)
    )) +
    facet_grid(.~pheno_group , space = "free",  scales = "free",
               labeller = facet_labels, switch="x") +
    theme_minimal() +
    ...
    geom_col(colour = "black", width = 0.6, position=position_dodge2(preserve = "single", 
                                                                     padding = 0)) +
    geom_errorbar(aes(ymin = !!sym(yvar_bar_lb), 
                      ymax = !!sym(yvar_bar_ub)), 
                  width = .6, position = position_dodge2(width = 0, 
                                                         preserve = "single",
                                                         padding = 0.5)) +
    ...
    xlab("") +
    ylab(TeX("Incremental $\\textit{R}^2$")) +
    scale_fill_manual(values = color_pal) +
    # scale_fill_viridis(discrete=TRUE) +
    annotation_custom(line, ymin = -7.3, ymax = -7.3) +
...
plot12 <- data %>%
    ...
    # scale_fill_viridis(discrete=TRUE) +
    coord_cartesian(clip = "off") +
    annotation_custom(line, ymin = -3, ymax = -3) +
...
plot2 <- data %>%
    ...
    scale_fill_manual(values = color_pal) +
    # scale_fill_viridis(discrete=TRUE) +
    annotation_custom(line, ymin = -2, ymax = -2) +
    coord_cartesian(clip = "off")
...
plot3 <- data %>%
    ...
    scale_fill_manual(values = color_pal) +
    # scale_fill_viridis(discrete=TRUE) +
    annotation_custom(line, ymin = -1.7, ymax = -1.7) +
    coord_cartesian(clip = "off")


# arranging plots
g11 <- ggplotGrob(plot11)
g11$grobs[[7]]$layout$clip <- "off"
...
plot_211 <- data %>%
    filter(complete.cases(!!sym(yvar_diff))) %>%
    filter(pheno_group %in% groups[1:3]) %>%
    filter(phenotype == "BMI" | phenotype == "HEIGHT") %>%
    ggplot(aes(x = phenotype_n, y = !!sym(yvar_diff), group = Dataset)) +
    facet_grid( .~pheno_group ,  space = "free",  scales = "free", switch="x",
                labeller = facet_labels) +
    theme_minimal() +
    theme(panel.grid.major.x = element_blank(),
          ...
          legend.title = element_text(size = 14),
          legend.position="none")  +
    geom_errorbar(aes(ymin = !!sym(yvar_diff_lb),
                      ymax = !!sym(yvar_diff_ub),
                      color = Dataset),
                  width = .5,
                  size = 0.5,
    ...
    scale_x_discrete(labels = setNames(make.x.labels(data$phenotype_n, 
                                                     "sample_single"), 
                                       data$phenotype_n)) +
    scale_y_continuous(breaks = seq(-6,15, 2),
                       limits = c(-7.1, 13),
                       expand = c(0, 0),
                       labels = function(x) paste0(x, "%")) +
    ...
    annotation_custom(line, ymin = -11, ymax = -11) +
    guides(shape = FALSE)
...
plot_212 <- data %>%
    ...
    scale_x_discrete(labels = setNames(make.x.labels(data$phenotype_n, 
                                                     "sample_single"), 
                                       data$phenotype_n)) +
    scale_y_continuous(breaks = seq(-4,15, 1),
                       expand = c(0, 0),
                       labels = function(x) paste0(x, "%")) +
    geom_hline(yintercept = 0, color = "black") +
    xlab("") +
    ...
    coord_cartesian(clip = "off") +
    annotation_custom(line, ymin = -6.9, ymax = -6.9) +
...
plot_22 <- data %>%
    ...
    coord_cartesian(clip = "off") +
    labs(shape = "Public Score") +
    annotation_custom(line, ymin = -4, ymax = -4) +
    guides(shape = FALSE)
...
plot_23 <- data %>%
    ...
    coord_cartesian(clip = "off") +
    labs(shape = "Public Score") +
    annotation_custom(line, ymin = -2.9, ymax = -2.9) +
    guides(shape = FALSE)


# arranging plots
g11 <- ggplotGrob(plot_211)
g11$grobs[[7]]$layout$clip <- "off"
