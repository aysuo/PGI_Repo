#----------------------------------------------------------------------------------#
# Charts differences in multi trait and public prediction results
# Date: 04/16/2019
...
packages <- c("data.table", "dplyr", "ggplot2", 
              ...
              "stringr", "viridis", "cowplot", "latex2exp")
...
data <- data %>%
    mutate(data_group2 = case_when(complete.cases(r2_diff_multi_public) ~ "Available",
                                   TRUE ~ "Unavailable"))
...
plot12 <- data %>%
    ...
    annotation_custom(line, ymin = -3.5, ymax = -3.5) +
...
plot3 <- data %>%
    ...
    scale_fill_manual(values = color_pal) +
    # scale_fill_viridis(discrete=TRUE) +
    annotation_custom(line, ymin = -1.5, ymax = -1.5) +
    coord_cartesian(clip = "off")
...
plot_212 <- data %>%
    ...
    coord_cartesian(clip = "off") +
    annotation_custom(line, ymin = -5, ymax = -5) +
...
plot_22 <- data %>%
    ...
    annotation_custom(line, ymin = -3.8, ymax = -3.8) +
    guides(shape = FALSE)
