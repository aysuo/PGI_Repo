...
cd $gf_dir_genotyped_ERisk/preImputationQC

awk 'NR==FNR{a[$1]=$2;next}($1 in a){$4=a[$1];print;next}{print}' ${gf_orig_genotyped_ERisk_Mothers}.fam ${gf_orig_genotyped_ERisk_Kids}.fam > Kids.fam

## Merge kids and mothers, do variant QC: Call rate> 0.98 , maf>0.01, hwe 1e-4
plink1.9 --bed ${gf_orig_genotyped_ERisk_Kids}.bed \
--bim ${gf_orig_genotyped_ERisk_Kids}.bim \
--fam Kids.fam \
--bmerge ${gf_orig_genotyped_ERisk_Mothers} \
--make-bed \
--out ERisk
...
sh $PGI_Repo/code/7_Genotypes/7.0_HRCimputation_prep.sh ERisk_autosome_geno02_maf01_hwe1e-4_sexcheck_hethom